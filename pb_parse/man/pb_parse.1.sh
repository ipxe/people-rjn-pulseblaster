...
BINARY=../src/pb_parse.php

#Description: brief string for the start of the man page.
DESCRIPTION="parser for pulseblaster code: covert pbsrc to vliw."

#Synopsis text, or leave blank to omit. Add leading spaces to avoid automatic paragraph formatting.
#SYNOPSIS=`cat <<-EOT
# pb_parse converts a human-readable (.pbsrc) input file to the pseudo-machine-code (.vliw) format for the Spincore PulseBlaster.
# It can parse, debug, verify, simulate (in realtime, to physical parallel ports) and assemble.
# This is a parser, not a compiler, because the PB hardware and VLIW instruction-set is not Turing-complete.
#EOT`
...
SEE_ALSO=( "pulseblaster (1)" "pb_utils (1)" "pb_parport-output (1)" "hawaiisim (1)" "gtkwave (1)" "pbsrc (5)" "vliw (5)" "pbsim (5)" )
