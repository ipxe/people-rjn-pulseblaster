...
BINARY="fudge.sh See: /usr/local/share/doc/pb_parse/pbsrc.txt"

#Description: brief string for the start of the man page.
DESCRIPTION="the PulseBlaster source file format"

#Synopsis text, or leave blank to omit. Add leading spaces to avoid automatic paragraph formatting.
SYNOPSIS=`cat <<-EOT
 A .pbsrc file is a pulseblaster-source file: the human-readable program for the Spincore PulseBlaster.
 It is intended to be converted to pseudo-machine code by pb_parse.
EOT`
...
SEE_ALSO=( "pb_parse (1)" "pb_utils (1)" "vliw (5)" "pbsim (5)" )
