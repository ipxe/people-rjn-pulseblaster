...
BINARY="fudge.sh See: /usr/local/share/doc/pb_utils/vliw.txt"

#Description: brief string for the start of the man page.
DESCRIPTION="the PulseBlaster Very Long Instruction Word file format"

#Synopsis text, or leave blank to omit. Add leading spaces to avoid automatic paragraph formatting.
SYNOPSIS=`cat <<-EOT
 A .vliw file is a pulseblaster instruction file. It is generated from a human-readable .pbsrc program by pb_parse.
 It can then be programmed into the pulseblaster using pb_asm or pb_prog.
 The Pulseblaster documentation is in: /usr/local/share/doc/pb_utils/
 The specification and opcodes are detailed in vliw.txt and pulseblaster-opcodes.txt
EOT`
...
SEE_ALSO=( "pb_utils (1)" "pb_parse (1)" "pb_vliw (1)" "pbsrc (5)" "pbsim (5)" )
...
#LICENSE="GPL v3+"
#AUTHOR="The author of $NAME and this manual page is Richard Neill, <pulseblaster@richardneill.org>"$'\n.br\n'"Copyright $DATE; this is Free Software ($LICENSE), see the source for copying conditions."
